...
// Load the sequence of scripts as they are loaded in index-catalog-defer.js
const filesToLoad = [
  'product-config.js',
  'product-utils.js',
...
// Now replicate the buildProductsFromCatalog and helper functions from index-home-app.js
...
function buildProductsFromCatalog() {
    ...
    console.log('Categories detected for homepage:', catKeys);
    ...
    while (more && safety < 600) {
        ...
        for (i = 0; i < catKeys.length; i++) {
            ...
            while (indices[key] < list.length) {
                ...
                if (globalSeen[imgKey]) {
                    console.log(`Skipping duplicate image key "${imgKey}" for product: "${p.name}" in category "${key}"`);
                    continue;
                }
...
console.log('\n--- HOMEPAGE PRODUCTS ---');
homepageProducts.forEach((p, idx) => {
  console.log(`${idx + 1}. [${p.category}] ${p.id} - ${p.name} - Price: ${p.price} - Image: ${p.img}`);
});
console.log(`Total homepage products: ${homepageProducts.length}`);
